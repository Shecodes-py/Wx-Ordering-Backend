...
from .responder import (
    ...
    reply_cart,
    reply_ask_notes,
    reply_ask_fulfillment,
    reply_fulfillment_selected,
    reply_unknown_fulfillment,
    reply_ask_address,
...
def process_message(menu_items: list, profile, session, user_message: str) -> dict:
    ...
    # Usually pre-filled from the platform's own display name (WhatsApp profile
    # name / Twilio ProfileName) at profile creation, so this rarely triggers.
    # When it does, ask explicitly and wait for the *next* message as the answer —
    # never treat whatever the customer happened to send first as their name.
    if not profile.full_name or profile.full_name.lower() in _LEGACY_NAMES:
        ...
        session.save(update_fields=['state'])
        return _result('GENERAL_CHAT', reply="Welcome to WX! 👋🍽️\n\nWhat's your name?")
    ...
    return _result(
        ...
        extracted_fulfillment_type=entities.get('fulfillment_type'),
...
def _generate_reply(intent, entities, msg, m, menu_items, menu_map, profile, session) -> str:
    ...
    if intent == 'PROVIDE_NOTES':
        ...
        fulfillment_reply = reply_ask_fulfillment()
        if notes:
            return f"Got it — noted: _{notes}_ ✅\n\n{fulfillment_reply}"
        return f"No special instructions — got it! ✅\n\n{fulfillment_reply}"

    if intent == 'SELECT_FULFILLMENT_TYPE':
        ftype = entities.get('fulfillment_type')
        if not ftype:
            return reply_unknown_fulfillment()
        confirmed = reply_fulfillment_selected(ftype)
        if ftype == 'DELIVERY':
            address_reply = reply_ask_address(saved_address=_real_address(profile.delivery_address or ''))
            return f"{confirmed}\n\n{address_reply}"
        # Pickup — no address needed. Next step: ask payment if not set
        if not session.payment_method:
            return f"{confirmed}\n\n{reply_ask_payment()}"
        return f"{confirmed}\n\n{_summary_reply(session, '', menu_map)}"
...
def _checkout_reply(session, profile, menu_map) -> str:
    ...
    if not session.fulfillment_type:
        return reply_ask_fulfillment()

    if session.fulfillment_type == 'DELIVERY' and not session.extracted_address:
...
def _result(intent, reply, items=None, extracted_notes=None,
            extracted_fulfillment_type=None, extracted_address=None,
            extracted_payment_method=None, is_confirmed=None) -> dict:
    return {
        'intent': intent,
        'items': items or [],
        'extracted_notes': extracted_notes,
        'extracted_fulfillment_type': extracted_fulfillment_type,
